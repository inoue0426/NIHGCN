...
class Optimizer(nn.Module, ABC):
    ...
    def forward(self):
        ...
        for epoch in range(self.epochs):
            ...
            train_loss = cross_entropy_loss(self.train_data, predict_data, self.train_mask)
            ...
            # Evaluation phase (executed every eval_freq epochs)
            if epoch % self.eval_freq == 0:
                ...
                with torch.no_grad():
                    ...
                    train_pred = torch.masked_select(predict_data, self.train_mask)
                    train_true = torch.masked_select(self.train_data, self.train_mask)
                    train_auc = self.evaluate_fun(train_true, train_pred)

                    # Evaluate on validation data
                    val_pred = torch.masked_select(predict_data, self.val_mask)
                    val_true = torch.masked_select(self.val_data, self.val_mask)
                    val_auc = self.evaluate_fun(val_true, val_pred)
        ...
        with torch.no_grad():
            ...
            test_pred = torch.masked_select(final_predictions, self.test_mask)
            test_true = torch.masked_select(self.test_data, self.test_mask)
            test_auc = self.evaluate_fun(test_true, test_pred)

            print(f"Training completed. Test AUC: {test_auc:.4f}")

        return test_true, test_pred, test_auc
